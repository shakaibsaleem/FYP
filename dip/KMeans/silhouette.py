...
def csvMakerLabel(file, nClusters):
    rd =csv.reader(open(file))
    a = main(file,nClusters)
    names = []
    labels = []
    i = 0
    for row in rd:
    ...
    # with open('Cluster_NoHuman_' + file[18:], 'w', newline='') as writeFile:
    #     writer = csv.writer(writeFile)
    #     writer.writerows(a)
...
def silhoutte(file):
    ...
    for n_cluster in range(2, 11):
        label = csvMakerLabel(file, n_cluster)
        # print(label)
...
# mainn()
print(silhoutte('haralick_no_human_grayscale.csv'))
ent is {}".format(n_cluster, sil_coeff))